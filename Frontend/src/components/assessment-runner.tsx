...
function AssessmentResult({ config, answered, correct, onReview, membership }: { config: AssessmentConfig; answered: number; correct: number; onReview: () => void; membership: Membership }) {
  return <div className="assessment-result"><header><p className="assessment-type">Hasil preview</p><h1>Try Out selesai</h1><p>Ringkasan ini memakai jawaban lokal dan bukan skor produksi.</p></header><section className="result-grid"><article className="score-card"><span className="score-ring"><strong>{correct}</strong><small>dari {config.questions.length} contoh</small></span><h2>Jawaban sesuai kunci mock</h2><p>Tidak ada status lulus karena passing score belum dikonfigurasi.</p></article><article className="result-detail"><div><span>Jawaban terisi</span><strong>{answered} / {config.questions.length}</strong></div><div><span>Jawaban sesuai</span><strong>{correct}</strong></div><div><span>Review</span><strong>{config.reviewEnabled ? "Tersedia" : "Tidak tersedia"}</strong></div><div><span>Timer</span><strong>Visual only</strong></div></article></section><section className="result-note"><span aria-hidden="true">灯</span><div><h2>Gunakan hasil sebagai preview tampilan.</h2><p>Skor, breakdown, rekomendasi, dan aturan hasil final harus berasal dari konfigurasi serta backend.</p></div></section><div className="result-actions"><Link href={`/dashboard?membership=${membership}`}>Kembali ke Dashboard</Link>{config.reviewEnabled && <button type="button" onClick={onReview}>Review jawaban →</button>}</div></div>;
}
