...
import { LuCheck, LuInfo, LuLightbulb, LuSend, LuX } from "react-icons/lu";
...
export function AssessmentRunner({ config, membership }: { config: AssessmentConfig; membership: Membership }) {
  ...
  return (
    <div className="assessment-page">
      ...
      {confirming && <div className="assessment-modal" role="presentation"><div className="modal-backdrop" onClick={() => setConfirming(false)} /><section role="dialog" aria-modal="true" aria-labelledby="submit-title"><span className="modal-icon" aria-hidden="true"><LuSend /></span><h2 id="submit-title">Kirim jawaban sekarang?</h2><p>Hasil akan menampilkan perolehan nilai dari jawaban yang dipilih.</p><div className="modal-summary"><span>Terjawab <b>{answered}</b></span><span>Belum dijawab <b>{config.questions.length - answered}</b></span></div><div className="modal-actions"><button type="button" onClick={() => setConfirming(false)}>Periksa kembali</button><button className="primary" type="button" onClick={() => { setConfirming(false); setView("result"); }}>Kirim Jawaban</button></div></section></div>}
    </div>
  );
}
...
function AssessmentResult({ config, answered, correct, onReview, membership }: { config: AssessmentConfig; answered: number; correct: number; onReview: () => void; membership: Membership }) {
  ...
  return <div className="assessment-result"><header><p className="assessment-type">Hasil evaluasi</p><h1>{config.completionLabel ?? "Try Out selesai"}</h1><p>Ringkasan perolehan nilai dan status evaluasi sesi.</p></header><section className="result-grid"><article className="score-card"><span className="score-ring"><strong>{score}</strong><small>skor perolehan</small></span><h2>{passing === null ? "Jawaban terkirim" : passing ? "LULUS" : "TIDAK LULUS"}</h2><p>{passing === null ? "Evaluasi selesai tanpa passing score khusus." : `Passing score minimal ${config.passingScore}.`}</p></article><article className="result-detail"><div><span>Jawaban terisi</span><strong>{answered} / {config.questions.length}</strong></div><div><span>Jawaban tepat</span><strong>{correct}</strong></div><div><span>Review</span><strong>{config.reviewEnabled ? "Tersedia" : "Tidak tersedia"}</strong></div><div><span>Status Sesi</span><strong>Tersimpan</strong></div></article></section><section className="result-note"><span aria-hidden="true"><LuLightbulb /></span><div><h2>Hasil evaluasi tersimpan resmi.</h2><p>Skor, breakdown, dan rekomendasi belajar dapat ditinjau kembali kapan saja.</p></div></section><div className="result-actions"><Link href={config.returnHref ?? `/dashboard?membership=${membership}`}>Kembali</Link>{config.reviewEnabled && <button type="button" onClick={onReview}>Review jawaban →</button>}</div></div>;
}

function AssessmentReview({ config, answers, onBack }: { config: AssessmentConfig; answers: Answers; onBack: () => void }) {
  return <div className="assessment-review"><header><button type="button" onClick={onBack}>← Kembali ke hasil</button><p className="assessment-type">Review jawaban</p><h1>Pembahasan Soal</h1><p>Status benar/salah memakai ikon dan teks informatif.</p></header><section>{config.questions.map((question, index) => { const selected = answers[question.id]; const isCorrect = selected === question.correctOptionId; return <article className={isCorrect ? "review-correct" : "review-incorrect"} key={question.id}><div className="review-status"><span aria-hidden="true">{isCorrect ? <LuCheck /> : <LuX />}</span><strong>{isCorrect ? "Jawaban benar" : selected ? "Jawaban belum tepat" : "Tidak dijawab"}</strong><small>Pertanyaan {index + 1}</small></div><h2>{question.prompt}</h2>{question.japanese && <ruby className="review-japanese">{question.japanese.text}<rt>{question.japanese.reading}</rt></ruby>}<div className="review-answer"><span>Jawabanmu</span><b>{question.options.find((item) => item.id === selected)?.label ?? "Tidak ada jawaban"}</b></div><div className="review-answer correct"><span>Jawaban tepat</span><b>{question.options.find((item) => item.id === question.correctOptionId)?.label}</b></div><p>{question.explanation}</p></article>; })}</section></div>;
}
