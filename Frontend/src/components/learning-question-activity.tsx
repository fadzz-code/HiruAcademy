...
export function LearningQuestionActivity({ data, variant }: { data: LearningData; variant: "audio" | "reading" }) {
  ...
  return <div className="learning-question-page">
    <header className="learning-question-head"><h1>{isAudio ? "Dengarkan percakapan dan pilih jawaban yang tepat" : "Baca teks pendek lalu jawab pertanyaan"}</h1><p>{isAudio ? "Audio dapat diputar ulang sesuai konfigurasi soal." : "Baca teks pendek dengan teliti lalu jawab pertanyaan."}</p></header>
    <div className="learning-question-layout"><main>
      {isAudio ? <section className="audio-player" aria-label="Audio fixture"><button type="button" aria-label="Putar audio">❚❚</button><div><strong>Percakapan tentang rutinitas pagi</strong><span>01:12 / 02:14</span></div></section> : <section className="reading-passage"><p className="dash-kicker">TEKS BACAAN</p><p lang="ja">{data.readingPassage}</p></section>}
