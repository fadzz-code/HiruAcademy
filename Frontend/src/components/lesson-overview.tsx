...
export function LessonOverview({ data }: { data: LearningData }) {
  ...
  return <>
    ...
    <section className="learning-progress-card"><div><p className="dash-kicker">CHAPTER PROGRESS</p><h2>2 dari 7 aktivitas selesai</h2><p>Lanjutkan video, dua modul, flashcard, audio, reading, lalu checkpoint.</p></div><div className="journey-progress"><span>Progress tersimpan</span><div><i /></div></div></section>
    ...
    {data.activities.find((activity) => activity.key === "checkpoint")?.href ? <section className="learning-checkpoint"><div><p className="dash-kicker">CHECKPOINT TERSEDIA</p><h2>Uji pemahaman Chapter {data.chapterNumber}</h2><p>Uji pemahaman materi chapter sebelum melanjutkan ke materi berikutnya.</p></div><Link href={data.activities.find((activity) => activity.key === "checkpoint")!.href!}>Mulai Checkpoint</Link></section> : <section className="learning-checkpoint"><div><p className="dash-kicker">CHECKPOINT TERKUNCI</p><h2>Selesaikan seluruh aktivitas sebelum checkpoint</h2><p>Status akan berubah otomatis setelah persyaratan chapter terpenuhi.</p></div><span><i aria-hidden="true">⌑</i>Terkunci</span></section>}
