...
export function FlashcardSession({ cards, membership, level, chapter }: { cards: FlashcardItem[]; membership: Membership; level: string; chapter: string }) {
  ...
  if (completed) return <div className="flashcard-complete"><header><p className="dash-kicker">FLASHCARD SELESAI</p><h1>Deck Chapter 4 sudah ditinjau</h1><p>Kartu yang sulit dapat diulang sebelum melanjutkan ke Audio Question.</p></header><section className="flashcard-complete-summary"><div><ruby>日本語<rt>にほんご</rt></ruby><span>Klik untuk melihat arti</span></div><div><strong>20 kartu selesai</strong><p>Sebagian besar kartu sudah dipahami. Ulangi kartu yang ditandai Sulit sebelum checkpoint.</p></div></section><section className="flashcard-confidence-summary">{[["13","Mudah"],["5","Sulit"],["2","Ulangi"]].map(([value,label]) => <div key={label}><strong>{value}</strong><span>{label}</span></div>)}</section><div className="flashcard-complete-actions"><Link className="flashcard-action-secondary" href={`/flashcards?membership=${membership}`}>Kembali ke Kumpulan</Link><button className="flashcard-action-secondary" type="button" onClick={() => { setRetrying(true); setIndex(0); setFlipped(false); setCompleted(false); }}>Ulangi Kartu Sulit</button><Link className="flashcard-action-primary" href={`/learn/${level}/${chapter}/audio?membership=${membership}`}>Lanjut Audio Question</Link></div><section className="flashcard-difficult"><h2>Kartu yang perlu diulang</h2><div>{cards.slice(0, 3).map((item) => <article key={item.id}><ruby>{item.term}<rt>{item.reading}</rt></ruby><span>Klik untuk melihat arti</span></article>)}</div></section><aside><strong>✓ Pengumuman</strong><p>Progress deck disimpan. Lanjutkan ke Audio Question atau ulangi kartu sulit.</p></aside></div>;

  return (
    <div className="flashcard-session">
      <header className="flashcard-head"><Link href={`/flashcards?membership=${membership}`}>Kembali ke Kumpulan</Link><div className="flash-progress"><strong>{index + 1} dari {cards.length} kartu</strong><i><b style={{ width: `${((index + 1) / cards.length) * 100}%` }} /></i></div></header><div ref={surfaceRef} className={`flashcard-surface${flipped ? " flipped" : ""}`} role="button" tabIndex={0} onClick={toggleCard} onKeyDown={(event) => { if (event.key === "Enter") toggleCard(); if (event.key === " ") { event.preventDefault(); toggleCard(); } }} aria-pressed={flipped}>
