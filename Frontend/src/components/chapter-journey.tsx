  ...
  return (
    <>
      ...
      <section className="chapter-list" aria-label={`Chapter ${level.title}`}>{chapters.map((chapter) => {
        ...
        return <article className={`chapter-card chapter-${chapter.state}`} key={chapter.key}><div className="chapter-marker"><span>{chapter.state === "completed" ? "✓" : chapter.state === "current" ? "日" : locked ? "⌑" : "○"}</span><i /></div><div className="chapter-copy"><div className="chapter-meta"><span>{chapter.statusLabel}</span><small>{chapter.orderLabel}</small></div><h2>{chapter.title}</h2><p>{chapter.description}</p><ul aria-label="Aktivitas chapter">{chapter.activities.map((activity) => <li key={activity}><span aria-hidden="true">{activityGlyph[activity]}</span>{activity}</li>)}</ul></div><div className="chapter-action"><span>{chapter.progressLabel}</span>{locked ? <Link href={membership === "free" ? "/#program" : `/journey/${level.slug}?membership=${membership}`}>{membership === "free" ? "Lihat akses" : "Belum tersedia"}</Link> : <Link className="chapter-action-button" href={`/learn/${level.slug}/${chapter.key}?membership=${membership}`}>{chapter.state === "completed" ? "Ulas Chapter" : chapter.state === "current" ? "Lanjutkan" : "Mulai"}<b aria-hidden="true">→</b></Link>}</div></article>;
      })}</section>
    </>
  );
}
