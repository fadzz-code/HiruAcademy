  ...
  return (
    <PublicPage>
      <main id="top">
        ...
        <aside className="proof-strip" aria-label="Pencapaian HIRU Academy"><div className="container proof-strip-grid">{proofItems.map((item) => <div className="proof-item" key={item.value}><span><Icon name={item.icon} width="24" height="24" /></span><p><strong>{item.value}</strong>{item.label && <small>{item.label}</small>}</p></div>)}</div></aside>

        <section className="section" id="program"><div className="container"><div className="section-heading"><h2>Pilih cara belajar yang paling sesuai</h2><p>Pilih cara belajar, lalu tentukan level N5–N1 secara bebas. Harga dan akses mengikuti konfigurasi sistem.</p></div><div className="offer-grid">{offers.map((offer, index) => <article className="offer-card" key={offer.title}><div className={`offer-icon offer-icon-${index + 1}`}><Icon name={offer.icon} width="28" height="28" /></div><span className="offer-badge">{offer.badge}</span><div className="offer-title"><h3>{offer.title}</h3>{offer.popular && <span className="popular-badge">Populer</span>}</div><p>{offer.description}</p><ul className="offer-points">{offer.points.map((point) => <li key={point}>{point}</li>)}</ul><a href={offer.href}>{offer.cta}</a></article>)}</div></div></section>
