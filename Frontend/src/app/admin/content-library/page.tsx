...
export default function ContentLibraryPage() {
  ...
  function duplicate() { const duplicateAsset = { ...selected, id: `${selected.id}-copy-${assets.length}`, title: `${selected.title} — Copy`, status: "Draft" }; setAssets((items) => [...items, duplicateAsset]); setSelectedId(duplicateAsset.id); setFeedback("Duplikasi tersedia pada contoh data."); }
  function archive() { setAssets((items) => items.map((asset) => asset.id === selected.id ? { ...asset, status: "Archived" } : asset)); setArchiveOpen(false); setFeedback("Asset diarsipkan pada contoh data; backend dan audit belum terhubung."); }

  return <AdminShell current="content"><main className="admin-page admin-a2-page">
    <header className="admin-header"><div><p className="admin-kicker">ADMIN • CONTENT LIBRARY</p><h1>Content Library Management</h1><p>Kelola video, modul PDF, flashcard, audio, reading, gambar, dan informasi materi.</p></div><div className="admin-header-actions"><button className="button button-primary" type="button" onClick={() => { setEditing(true); editorRef.current?.scrollIntoView({ behavior: "smooth" }); }}>Tambah Konten</button><Link className="button button-dark" href="/admin/program/n4/chapters">Chapter Builder</Link></div></header>

    <section className="admin-kpi-grid">{[["Published assets","148 File","Konten aktif dan siap dipelajari."],["Draft assets","24 File","Menunggu tinjauan kelengkapan."],["Validation issues","0 Masalah","Seluruh media tervalidasi."],["Jepang Pemula","86 Modul","Konten dengan bantuan furigana."]].map(([label,value,meta]) => <article className="admin-kpi-card" key={label}><h2>{label}</h2><strong>{value}</strong><small>{meta}</small></article>)}</section>

    <section className="library-filter-panel library-filter-panel-reorder">
      <label className="admin-search-box library-search-left">
        <span aria-hidden="true">⌕</span>
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari judul, tipe, level, atau chapter" />
      </label>
      <label className="admin-field library-dropdown-field">
        <span>Tipe Materi</span>
        <select className="admin-bordered-select" value={type} onChange={(event) => setType(event.target.value)}>
          {["Semua", "Video", "PDF Modules", "Flashcard", "Audio", "Reading", "Images"].map((item) => (
      ...
      <label className="admin-field library-dropdown-field">
        <span>Level</span>
        <select className="admin-bordered-select" value={level} onChange={(event) => setLevel(event.target.value)}>
      ...
      <label className="admin-field library-dropdown-field">
        <span>Status</span>
        <select className="admin-bordered-select" value={status} onChange={(event) => setStatus(event.target.value)}>
