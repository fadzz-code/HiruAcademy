    ...
    <section className="blog-hero"><div><p className="kicker">HIRU INSIGHT</p><h1>Panduan Bahasa Jepang dan Persiapan JLPT</h1><p>Temukan penjelasan materi, strategi ujian, budaya Jepang, dan tips belajar praktis dari Hiru Academy.</p><label className="blog-search"><span aria-hidden="true">⌕</span><input aria-label="Cari materi, level, atau topik" placeholder="Cari materi, level, atau topik" value={query} onChange={(event) => setQuery(event.target.value)} /></label></div></section>
        <section className="blog-featured">
          ...
          <div className="blog-featured-content">
            <p className="kicker">{blogFeatured.label}</p>
            <ArticleMeta category={blogFeatured.category} author={blogFeatured.author} publishedAt={blogFeatured.publishedAt} />
            <h2>{blogFeatured.title}</h2>
            <p>{blogFeatured.description}</p>
            <Link className="button button-primary" href={`/blog/${blogDetailSlug}`}>Baca Artikel</Link>
          </div>
        </section>
        <section className="public-section blog-latest">
          ...
          <div className="blog-filters" aria-label="Filter kategori">
            {categories.map((item) => (
              <button className={category === item ? "active" : ""} type="button" aria-pressed={category === item} onClick={() => setCategory(item)} key={item}>{item}</button>
            ))}
          </div>
              ...
              {visibleArticles.map((article) => (
                <article className="blog-card" key={article.title}>
                  ...
                  <div className="blog-card-body">
                    ...
                    <Link className="button button-outline" href={`/blog/${blogDetailSlug}`}>Baca Artikel</Link>
                  </div>
                </article>
              ))}
            </div>
