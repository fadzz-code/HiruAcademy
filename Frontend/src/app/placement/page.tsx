...
export default function PlacementPage() {
  ...
  return <PublicPage active="Placement Test"><main className="public-main placement-page">
    ...
    <section className="placement-form-card placement-form-focus" id="placement-form"><form className="public-form" action="/placement/question"><p className="kicker">DATA SEBELUM TES</p><label>Nama<input name="name" placeholder="Masukkan nama" value={name} onChange={(event) => setName(event.target.value)} required /></label><label>Nomor WhatsApp<input name="whatsapp" type="tel" inputMode="tel" autoComplete="tel" placeholder="+62 8xx xxxx xxxx" value={whatsapp} onChange={(event) => setWhatsapp(event.target.value)} required /></label><label>Target Ujian<select name="target" value={target} onChange={(event) => setTarget(event.target.value)} required><option value="" disabled>Pilih target ujian</option>{["Belum menentukan", "N5", "N4", "N3", "N2", "N1"].map((level) => <option key={level}>{level}</option>)}</select></label><label className="consent"><input type="checkbox" checked={privacy} onChange={(event) => setPrivacy(event.target.checked)} required /><span>Saya menyetujui pemrosesan data sesuai <Link href="/kebijakan-privasi">Kebijakan Privasi</Link>.</span></label><label className="consent"><input type="checkbox" name="whatsappConsent" /><span>Saya bersedia menerima penjelasan hasil dan informasi program Hiru Academy melalui WhatsApp.</span></label><button className="button button-primary" type="submit" disabled={!valid}>Mulai Placement Test Gratis</button></form></section>
  </main></PublicPage>;
}
